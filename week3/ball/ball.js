...
function start() {
    ...
    setInterval(() => {
        ...
        //hört auf wenn x & y zu klein ist
        nextBoard();
        display(context);
    }, 1000 / 20);
}
...
function nextBoard() {
    ...
    if (ball.y >= 390 && ball.dy > 0) {  // ball.y < 0 cannot occur due to conservation of energy
        ball.dy -= 4;
        ball.dy *= -1; // Vorzeichen umdrehen in andere Richtung springen
    }
    if (ball.x <= 10 && ball.dx < 0 || ball.x >= 390 && ball.dx > 0) {
        ball.dx *= -1; //x verliert an Geschwindigkeit
        ball.dx *= 0.8;
    }
    ball.x  += ball.dx;
    ball.y  += ball.dy;
    ball.y = Math.min(390, ball.y);
    ball.dy += 1.5 ;      // konstante Beschleunigung immer schneller
}
